...
|---|---|---|---|
| Host | `dotnet run`, ports fixed in `launchSettings.json` | Windows App Service `app-orlandoup-stg` on Ronatrip's existing Basic plan (D25) | Windows App Service `app-orlandoup` on the same plan until the D25 trigger, then its own plan |
...
managed certificate on App Service (supported on the Basic tier); DNS at the registrar (open
question which). Hosting starts on Ronatrip's existing Windows plan (D25): deploy is
`azure/webapps-deploy` with a zip package, Web Deploy from Visual Studio as the fallback. The
